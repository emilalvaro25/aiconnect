...
export class AudioStreamer {
  ...
  async addWorklet<T extends (d: any) => void>(
    ...
    worklet.port.onmessage = (ev: MessageEvent) => {
        workletsRecord![workletName].handlers.forEach(handler => {
            handler.call(worklet.port, ev);
        });
    }
  ...
  stop() {
    ...
    setTimeout(() => {
      // Re-create the gain node to clear any audio graph state
  ...
  setVolume(level: number) {
    if (this.gainNode) {
      // Using exponential ramp for a more natural volume change
      this.gainNode.gain.exponentialRampToValueAtTime(Math.max(0.0001, level), this.context.currentTime + 0.1);
    }
  }
